...
2) 특정 토픽에 쌓인 메시지 전부 조회하기 + 실시간으로 메시지 조회하기
    ...
    # 위 명령어를 입력하면 토픽에 저장된 메시지를 전부 조회한다. 그리고 계속해서 메시지를 실시간으로 조회하는 상태로 변경된다. 
# 정말 그런지 테스트하기 위해 터미널 창을 하나 더 켜서 `email.send` 토픽에 메시지를 추가로 넣어보자.
