...
1\. CD 작업순서
--------
Continuous Deployment(지속적 배포) 또는 Continuous Delivery(지속적 전달)의 약자입니다.
```
1) 이전 버전 종료             : 배포 서버에서 현재 돌아가고 있던 구버전 애플리케이션을 안전하게 종료합니다.
2) 최신 아티팩트 다운로드     : Nexus 저장소에 접속하여 이번에 CI 단계에서 새로 만들어진 최신 버전의 .jar 파일이나 Docker 이미지를 
                            배포 서버로 가져옵니다.
3) 새 버전 실행              : 다운로드한 새 .jar 파일을 java -jar 명령어로 실행하거나, Docker 명령어로 새 컨테이너를 띄웁니다 
                            (docker run).
4) 상태 검사(Health Check)  : 새 프로그램이 오류 없이 정상적으로 손님을 받을 준비가 되었는지 확인합니다.
```
