...
2\. 일반 프로젝트에서 많이 쓰는 CI/CD 구축 방법 (Github Actions, SCP)
--------
```
Github ▶ Github Actions ▶ EC2
  ↓_________↑    ↓_________↑
...
- 무중단 배포를 구현하거나 여러 EC2 인스턴스에 배포를 해야 하는 상황이라면, 직접 Github Actions에 스크립트를 작성해서 구현해야 한다. 
  직접 구현을 해보면 알겠지만 생각보다 꽤 복잡하다.

### ✅이 방법은 언제 주로 쓰는 지
- 현업에서 초기 서비스를 구축할 때 이 방법을 많이 활용한다.
    - 처음 서비스를 구현할 때는 대규모 서비스에 적합한 구조로 구현하지 않는다. 즉, 오버 엔지니어링을 하지 않는다. 
...
3\. 확장성을 고려한 프로젝트에서 많이 쓰는 CI/CD 구축 방법 (Code Deploy)
--------
```
                                5. S3로부터 빌드 파일을 다운 후 배포 진행
Github ▶ Github Actions ▶ AWS S3 →-------------------------------→ EC2
     ↓___↑     ↓      ↓____↑                                        ↑
 1.Git push    ↓      2.빌드 파일을 전달                               ↑
               ↓                                                    ↑ 4. EC2한테 S3로부터 빌드 파일을 다운 받은 뒤 배포를 진행하도록 명령
   3. 배포를 진행하라고 명령                                            ↑
               ↓                                                    ↑
       AWS CodeDeploy →---------------------------------------------→
...
4\. 컨테이너 기반의 프로젝트에서 많이 쓰는 CI/CD 구축 방법 (Docker)
--------
```
Github ▶ Github Actions ▶ AWS ECR →-----------------------------------
     ↓___↑     ↓      ↓____↑                                         ↓
 1.Git push    ↓      2.Docker Image 생성                             ↓
               ↓      3.Docker Image를 ECR로 전달                      ↓ 5. Docker Image 다운 후 배포 진행
   4. ECR로부터 Docker Image를 다운받아 배포를 진행하도록 명령              ↓
               ↓                                                     ↓
              EC2 ←---------------------------------------------------
...
5\. 컨테이너 기반 + 확장성을 고려한 프로젝트에서 많이 쓰는 CI/CD 구축 방법 (Docker, CodeDeploy)
--------
```
Github ▶ Github Actions ▶ AWS ECR →--------------------------------------------------------------------------------
     ↓___↑     ↓      ↓____↑                                                                                      ↓
 1.Git push    ↓      2.Docker Image 생성                                                                          ↓
               ↓      3.Docker Image를 ECR로 전달                                                                   ↓
               ↓                                                                                8. Docker Image 다운 후 배포 진행
               ↓_____________________                                                                              ↓
              ↙                      ↘                                                                             ↓
   4. CodeDeploy 관련 파일 업로드       5. 배포를 진행하라고 명령                                                        ↓
           ↙                            ↘                                                                          ↓
        AWS S3                      AWS CodeDeploy --------------------------------------------------------------> EC2
           ↓                              6. EC2한테 S3로부터 CodeDeploy 관련 파일을 다운 받은 뒤 배포를 진행하도록 명령    ↑
           ↓                                                                                                       ↑
           ---------------------------------------------------------------------------------------------------------
                                    7. CodeDeploy 관련 파일 다운
