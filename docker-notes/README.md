...
-----------------------------
```
Docker Desktop     : 직원 수 250명 이상이거나 연 매출 1,000만 달러 이상인 기업에서 사용할 경우 유료 라이선스(Docker Business)가 필요합니다.
linux에서 사용      : 완전 무료(Open Source)
```
...
* 컨테이너의 독립성 : 독립적인 컴퓨터 환경
    1) 디스크(저장 공간)   : 각 컨테이너마다 서로 각자의 저장 공간을 가지고 있다. 일반적으로 A 컨테이너 내부에서 B 컨테이너 내부에 있는 파일에 
                          접근할 수 없다. 
...
5\. Dockerhub
-----------------------------
공용 Docker Image 관리 서비스를 제공하고 있다. Hub에 올리는 이미지는 누구나 접근 가능하다.
```
