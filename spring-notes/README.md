...
> PSA : 트랜잭션의 분리(AOP)를 이해하셨다면 **PSA(Portable Service Abstraction)**는 그 연장선에 있는 아주 중요한 개념입니다.
    
    PSA는 **"환경과 세부 기술에 변화에 관계없이 일관된 방식으로 기술에 접근할 수 있게 해주는 추상화 계층"**을 의미합니다.
    
    1. PSA의 핵심 개념: "잘 만든 인터페이스"
    PSA의 핵심은 특정 기술에 종속되지 않도록 추상화(Abstraction) 계층을 두는 것입니다.
    
    예를 들어, 서비스에서 트랜잭션을 처리할 때 JDBC를 쓸 수도 있고, JPA나 Hibernate를 쓸 수도 있습니다. 만약 PSA가 없다면 기술을 바꿀 때마다 비즈니스 로직의 코드를 다 수정해야 합니다. 하지만 스프링은 이를 인터페이스로 한 번 감싸서 제공하기 때문에, 우리는 내부 구현이 바뀌어도 똑같은 방식으로 코드를 작성할 수 있습니다.
    ...
    사용자 코드: 어떤 기술을 쓰든 @Transactional 하나로 해결됩니다. 내부적으로는 PlatformTransactionManager라는 인터페이스를 통해 일관된 방식으로 트랜잭션을 관리하기 때문입니다.
    
    ② 스프링 웹 MVC (@Controller, @RequestMapping)
    우리는 서블릿(Servlet) 기반의 톰캣(Tomcat) 위에서 구동되는 코드를 짜지만, 정작 코드 어디에도 HttpServlet을 상속받거나 로우 레벨의 서블릿 API를 직접 다루는 일은 거의 없습니다. 스프링이 웹 기술을 추상화해두었기 때문입니다. 덕분에 나중에 서버를 네티(Netty) 기반의 WebFlux로 바꿔도 프로그래밍 모델은 거의 그대로 유지됩니다.
    
    ③ 캐시(Cache) PSA (@Cacheable)
    데이터를 Redis에 저장하든, Ehcache나 Caffeine에 저장하든 개발자는 @Cacheable이라는 추상화된 애노테이션만 사용하면 됩니다. 설정 파일에서 어떤 캐시 구현체를 사용할지만 바꿔 끼우면 로직 수정 없이 기술 변경이 가능합니다.
    ...
    PSA: 그 부가 기능이 어떤 하위 기술(JDBC, JPA 등)로 구현되든 **일관된 방식(What)**으로 사용할 수 있게 해주는 설계 원칙.
