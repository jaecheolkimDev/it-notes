...
1\. 스트림
-----------
프로그램과 I/0 객체를 연결하여 데이터를 송수신 하는 길이다.
```
...
2\. writer (BufferedWriter, FileWriter)
-----------
```
flush() : 남아있던 데이터를 파일에 완전히 밀어 넣음
close() : 마지막에 남아있던 데이터를 파일에 완전히 밀어 넣고(flush) 닫힙니다.
```
...
-----------
```
따라서 외부 자원(I/O)을 다룰 때는 null 체크만으로 안심하면 안 되며, 언제든 예외가 발생할 수 있다고 가정하고 try-catch 블록으로 꼼꼼하게 감싸야함
null체크해도
```
