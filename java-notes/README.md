...
- 상속(Inheritance)
    ...
    - super()   : 부모 클래스의 생성자를 호출하기 위해서 사용
                - 첫줄에 사용되어야 함
                    - 없다면 컴파일러가 자동으로 부모 클래스의 기본생성자를 호출(부모클래스에 기본 생성자가 없다면 에러 발생)
...
- final : reference를 변경 불가능하게 하는 명령어 ( 3 군데에서 사용 )
...
- process : 실행 동작의 단위
    ...
    - thread : 하나의 process에서 실행동작의 단위
        - 경쟁관계
        - Synchronized(작업이끝날때까지 할당받는것)가 필요할 때도 있음.
        ...
        - 스레드 객체를 생성하는 두가지 방식
            ...
            - Runnable 인터페이스를 상속
            - priorty : 우선순위 설정 가능.
            - synchronized (this) : 공유자원을 동시에 사용하지 못하도록 방지함 (단일 JVM(Java Virtual Machine) 내에서만 유효한 메커니즘입니다.)

              	yield() : 양보 - 블럭상태에 빠지는건 아님.
              	sleep() : 	   잠들면서 os에게 권한을 주고 깰 때 요구함.
              	th1.join()  :	 join을 사용한 쓰레드가 작업 완료할때까지 다른 스레드는 대기해야함.(쓰레드간 실행순서 제어)
              	wait()  :    쓰레드를 일시정지 상태로 만듬. / 메소드에 Synchronized키워드가 같이 사용되어져야함.
              	notify():	 쓰레드가 실행대기 상태로 변경. / 메소드에 Synchronized키워드가 같이 사용되어져야함.
              	notifyAll() :쓰레드가 실행대기 상태로 변경. / 메소드에 Synchronized키워드가 같이 사용되어져야함.
              	Synchronized() : lock을 얻고, 끝나면 lock을 양보함. / 쓰레드간 동기화를 보장하기 위해 사용됨. / 객체의 모니터 락을 획득하고 해제
...
- 정규표현식
    ...
    - java.util.regex.Pattern
        ...
        - Pattern.asPredicate() : 문자열을 일치시키는데 사ㅂㅂ
        - 용할 수 있는 Predicate 작성
...
5\. 리플렉션(Reflection)
-------------------
구체적인 클래스 타입을 알지 못하더라도, 컴파일된 바이트 코드를 통해 그 클래스의 메서드, 타입, 변수(Field)들을 런타임(실행 중)에 
거울 보듯이 들여다보고 조작할 수 있게 해주는 자바의 API입니다.
```
instanceof vs Reflection 핵심 차이

구분          instanceof 연산자                         리플렉션 (Reflection API)
주 목적        타입 확인 (이 타입이 맞니?)                 구조 분석 및 동적 제어 (뭐뭐 가지고 있니?)
확인 대상      특정 클래스/인터페이스의 인스턴스인지 여부     클래스의 메서드, 필드, 생성자, 애노테이션 등 전체
비공개 접근    불가능 (일반적인 접근 제어자 따름)            가능 (private 변수나 메서드도 강제로 제어 가능)
작동 예시      if (reader instanceof AutoCloseable)    Method[] methods = reader.getClass().getDeclaredMethods();
```
