...
------------------
```
공유 자원: 스레드 안전(Thread-safe)하지 않은 공유 객체(ItemReader, ItemWriter, ItemProcessor 내 변수 등)를 사용할 경우 
데이터가 꼬이게 됩니다.
 => lock을 걸거나.
 => 분리해서 처리할 수 있는것끼리 분리.
...
6\. 배치 커밋 단위에 따른 실패 대응 방안
------------------
```
1) 재시작 가능성(Restartability) 확보:
Spring Batch 등에서는 각 스텝의 성공/실패 상태를 DB(Meta-Table)에 기록합니다. 배치가 실패한 후 다시 실행하면, 성공한 Step 1은 
건너뛰고 실패한 Step 2부터 자동으로 재실행할 수 있도록 설계합니다.
2) 멱등성(Idempotency) 보장:
실패한 스텝을 다시 실행하거나 보상 작업을 진행할 때, 몇 번을 실행해도 결과가 동일하도록 설계합니다. (예: 이미 처리된 데이터는 
UPDATE하지 않고 건너뛰기)
3) 보상 트랜잭션 (Compensation) / 롤백 스텝 설계:
후속 스텝이 실패했을 때, 이미 커밋된 이전 스텝의 작업을 원복하는 전용 보상 스텝(예: Step 1에서 생성된 임시 데이터 삭제 또는 취소 
API 호출)을 실패 핸들러에 연결하여 실행합니다.
4) 단일 트랜잭션으로 묶기 (스텝 분리 지양):
원자성(All or Nothing)이 절대적으로 중요한 비즈니스 로직(예: 계좌 이체, 결제 정산 등)은 여러 스텝으로 쪼개지 않고 하나의 
스텝/트랜잭션 내부에서 처리합니다.
```
