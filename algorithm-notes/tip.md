...
14. [나머지 연산의 활용, %]
    - 반복되는 주기(빨간불+초록불) 문제는 언제나 % 연산자가 정답입니다.

15. [방향]
    - 좌표평면과 방향이 다름
    방향          설명          좌표 변화 (r,c)     이유
    북 (North)   위로 한 칸      $(-1, 0)$       위로 갈수록 행(r) 번호가 줄어들기 때문
    남 (South)   아래로 한 칸     $(+1, 0)$       아래로 갈수록 행(r) 번호가 늘어나기 때문
    동 (East)    오른쪽 한 칸     $(0, +1)$       오른쪽으로 갈수록 열(c) 번호가 늘어나기 때문
    서 (West)    왼쪽 한 칸      $(0, -1)$       왼쪽으로 갈수록 열(c) 번호가 줄어들기 때문
16. 
----------------------------------------------------------------------------------------------------
시간초과 이슈
